...
(i) Let us show that $\ppp^{(n)}$ is the smallest $\ppp$-primary ideal containing $\ppp^n$. We have $(\ppp^{(n)})_\ppp=\ppp_\ppp^n\subset\ppp_\ppp$. As $\ppp_\ppp$ is maximal, the ideal $(\ppp^{(n)})_\ppp$ is $\ppp_\ppp$-primary. Using contraction we see that $\ppp^{(n)}$ is $\ppp$-primary. Assuming $\ppp^n\subset\qqq\subset\ppp$ with $\qqq$ a $\ppp$-primary ideal, we get $\ppp_\ppp^n\subset\qqq_\ppp\subset\ppp_\ppp$, and $\ppp^{(n)}\subset\qqq\subset\ppp$. %If we had $\ppp^n\subset\qqq<\ppp$ with $\qqq$ a $\ppp$-primary ideal, localization would give $\ppp_\ppp^n\subset\qqq_\ppp<\ppp_\ppp^n$.
