...
\end{equation}
Assume \eqref3 is false, and let $\gamma$ be the largest multi-index satisfying $a_\gamma g\neq0$. Then \eqref1 implies $a_\gamma b_\beta=0$ [because $a_\gamma b_\beta\ne0$ would imply $\deg(fg)=\gamma+\beta$, but we have $fg=0$] and thus $\deg a_\gamma g<\beta$. As $f\cdot(a_\gamma g)=0$, this entails $a_\gamma g=0$, a contradiction. This proves \eqref3 and thus \eqref2. 
...
\subsubsection{Exercise 1.5iii}%\subsection{Page 11, Exercise 1.5iii}%

Other wording: $\mf R(A[[x]])=\mf R(A)+(x)$.

\subsubsection{Exercise 1.5iv}%\subsection{Page 11, Exercise 1.5iv}
...
(d) $\mmm=\mmm^{\oo c}+(x)$. Proof: Let $f\in A[[x]]$ with $f=a_0+a_1x+\cdots$. There is a unique $g\in A[[x]]$ such that $f=a_0+xg$, and (a) implies that $f\in\mmm$ if and only if $a_0\in\mmm^{\oo c}$. %then $f\in\mmm$ by (a). $f=a_0+xg\in\mmm$, then $a_0\in\mmm^{\oo c}$ by (a).

\subsubsection{Exercise 1.5v}%\subsection{Page 11, Exercise 1.5v}%
...
An absolutely flat local ring is a field: This follows immediately from Property~(iv) above (Section~\ref{227}) and Exercise 1.12 p.~11 of the book (see Section~\ref{112} p.~\pageref{112} above). %Let $(A,\mmm)$ be our local ring and let $\mu$ be in $\mmm$. It suffices to show $\mu=0$. If we had $\mu\ne0$, we would get $(\mu)=(e)$ for some nonzero idempotent $e$ (see end of Section \ref{227} above). Then $e\in\mmm=\mf R$ would imply that $f:=1-e$ is a unit, hence $f=f^2$ entails $f=1$ and thus $e=0$, a contradiction. 
...
(A_\ppp/\ppp_\ppp)\otimes_AB\simeq 
B_\ppp/\ppp B_\ppp=
B_\ppp/(\ppp_\ppp)^{\oo e}=
B_\ppp/(\ppp^{\oo e})_\ppp\simeq 
\big(B/\ppp^{\oo e}\big)_\ppp=
(B/\ppp B)_\ppp.
\end{equation} 
...
\textbf{Statement.} A multiplicatively closed subset $S$ of a ring $A$ is said to be \emph{saturated} if $xy\in S\iff x\in S$ and $y\in S$. %XY E S ~ X E Sand y E S.
...
$$ 
in $Aa^{-1}\otimes_AM$. This implies $ax=0$ because the map $M\to Aa^{-1}\otimes_AM$, $y\mapsto a^{-1}\otimes ay$ is an isomorphism. %If $1\otimes x=0$ in $K\otimes M$, then $1\otimes x=0$ in $A\frac1a\otimes M$. This implies $ax=0$.
...
\textbf{Statement.} Let $A$ be a ring. %, and let $F$ be the $A$-module $A^n$. 
...
$$
0\to N/\mmm N\to k^n\to k^n\to0.%\overset\sim
$$ 
...
$$ 
Now Exercise 2.12 p.~32 of the book shows that $N$ is finitely generated. %and Exercise 2.2 p.~31 of the book gives $k\otimes N=(A/\mmm)\otimes N\simeq N/\mmm N=0$. 
...
Note~\ref{sat1} p.~\pageref{sat1} and Note~\ref{sat2} p.~\pageref{sat2} imply (i), (ii), (iii) and (iv). Part (v) results from the following more general statement, whose proof is left to the reader:
...
\subsection{Page 48, Exercise 3.27}%Examples 25 and 26
...
The claim implies that there are infinitely many minimal prime ideals. %Using Urysohn's Lemma it is easy to see that any prime ideal is contained in a \emph{unique} maximal ideal. This implies that there are infinitely many minimal prime ideals.
...
If $f:A\to A[x]$ is the natural embedding, %let $$f^*:\Spec(A[x])\to\Spec(A)$$ be the continuous map induced by $f$, and let $\ppp$ be a prime ideal of $A$. T
...
$$ 
where $k$ is the residue field at $\ppp$. %we have $$f^{*-1}(\ppp)=\{\ \ppp[x]\ ,\ \ppp[x]+(x)\ \}.$$
...
Part (iii): Use Exercises 1.18 p.~10, 1.2ii p.~11 and 1.2iii p.~11. %Note that we have (by Exercise 1.2ii p.~11)\begin{equation}\label{rax}r(\aaa[x])=r(\aaa)[x]\end{equation}(obvious notation).
...
(iii) %Assuming $\ppp\supset\ppp'$, we must show $S_\ppp(0)\subset S_{\ppp'}(0)$. 
We have 
$$
...
$$ 
contradicting the maximality of $(\aaa:x)$.%and thus $(\aaa:xz)=(1)$, which is impossible since $xz\notin\aaa$.
...
%We claim $A_\ppp\cap\qqq_\ppp=\ppp_\ppp$. Set $S:=A\setminus\ppp$. As $S\cap\qqq=S\cap A\cap\qqq=S\cap\ppp=\varnothing$, we see that $\qqq_\ppp$ is prime, and it suffices to show $\ppp_\ppp\subset\qqq_\ppp$, which is clear.
...
%Let us prove the ``if'' part. Let $(B,g)\in\Sigma$ with $B$ a valuation ring of $K$, and 
\begin{proof}[Proof of Proposition \ref{521}] 
If we had $(B,g)<(C,h)$ for some $(C,h)\in\Sigma$, we would get $C=(\Ker g)C$ by Lemma~\ref{vrok}, and $(\Ker g)C\subset\Ker h$ by assumption. %and let $c$ be in $C\setminus B$. Then $c^{-1}$ is a non-unit of $B$. %Lemma 5.19 p.~65 of the book implies that
%As $B$ is local with maximal ideal $\Ker g$, the element $c^{-1}$ is in $\Ker g$, and thus in $\Ker h$, which is impossible.
\end{proof}
...
\emph{In the setting of Section~\ref{520} above, let $x$ be a non-zero element of $K$. If $\mmm[x]\ne B[x]$, then there is a maximal ideal $\mmm'$ of $B[x]$ such that $B\cap\mmm'=\mmm$ %containing $\mmm$ 
and $B[x]/\mmm'$ is algebraic over $B/\mmm$. If $\mmm[x^{-1}]\ne B[x^{-1}]$, then there is a maximal ideal $\mmm'$ of $B[x^{-1}]$ such that $B\cap\mmm'=\mmm$ %containing $\mmm$ 
...
%``Then the restriction to $A$ of the natural homomorphism $A'\to k'$ defines a homomorphism of $A'$ into $\Omega$. 
``By (5.21) the natural homomorphism $A'\to\Omega$ can be extended to some valuation ring $B\supseteq A'$.'' %Then the restriction to $A$ of the natural homomorphism $A'\to k'$ defines a homomorphism of $A'$ into $\Omega$. By (5.21) this can be extended to some valuation ring $B\supseteq A'$.
...
\textbf{Solution.} %The characteristic of $k$ must be $\ne2$. 
...
Proof 2. By Noether's Normalization Theorem applied to the $k$-algebra $A_f$, there are elements $u_1,\dots,u_m$ of $A_f$ which are algebraically independent over $k$, such that $A_f$ is integral over $k[u_1,\dots,u_m]$. By Exercise 5.2 %Theorem 5.10 
...
Part (i). Let us prove %Here is a partial solution.
...
We can assume that $B$ is a domain. Let $\bbb\subset B$ be the Jacobson radical of $B$. It suffices to prove $\bbb=(0)$. We have %Claim: 
$\bbb^{\oo c}=(0)$ by Exercise 5.5ii p.~67 of the book (see Section~\ref{55} p.~\pageref{55} above), and %Proof of the claim. By contradiction let $0\ne a\in\bbb^{\oo c}$, and let $\mmm$ be a maximal ideal of $A$ avoiding $a$. By Theorem 5.10 p.~62 of the book, there is a prime ideal $\nnn$ of $B$ lying above $\mmm$. Then $\nnn$ is maximal by Corollary 5.8 p.~61 of the book, and $a\in\nnn\cap A=\mmm$, a contradiction.\\ %In view of 
...
(3)$\implies$(1): for $U,V$ open and $V\subset U$ the equality $X_0\cap(U\setminus V)=\varnothing$ implies $X_0\cap U=X_0\cap V$.% for $U,V$ open and $V\subset U$.
...
%Let $C$ be a minimal element of $\Sigma$. Then $C$ is nonempty, and, for $c\in C$, we have $C=(C\setminus\{c\})\cup\overline{\{c\}}$. 
...
%$\quad\bu$ \emph{it has finitely many minimal elements} $\ppp_1,\ldots,\ppp_n,$

%$\quad\bu$ \emph{we have} $C=V(\ppp_1)\cup\cdots\cup V(\ppp_n)$.
...
(i) $\implies$ (ii): Use %Theorem 4.5 p.~52 (the First Uniqueness Theorem) and 
...
\textbf{Hint.} Let $A$ be the prime subring of $K$. Then $A=\mathbb Z$ or $A=\mathbb F_p$. Noether Normalization Theorem, as stated in Section~\ref{nnt} p.~\pageref{nnt}, and Proposition 5.7 p.~61 of the book imply that the first case is impossible and that, in the second case, $K$ is a finite degree extension of $\mathbb F_p$. %$K$ is a finitely generated module over $\mathbb Z[s^{-1}]$ for some $0\ne s\in\mathbb Z$
...
\end{array}
$$ 
%$$\begin{matrix}\aaa\ne(1)&x\in A&\qqq_i&\ppp_i=r(\qqq_i)&\aaa_i&\ppp_i^m\subset\qqq_i&\aaa_i\cap\ppp_i^m\\ \\ N<M&x\in M&Q_i&\ppp_i=r(Q_i:M)&N_i&\ppp_i^mM\subset Q_i&N_i\cap\ppp_i^mM\end{matrix}$$ 
...
$$ 
and $p\ge1$ is minimum for this condition. It suffices to show $p=1$. We do this by applying Lemma~\ref{719L3}b with $U:=U_1$, $V:=U_2$, $U':=U_2\cap V_{i_1}$, $V':=V_{i_2}\cap\cdots\cap V_{i_p}$. We get %$W=U_2\cap V_{i_1}$ or 
$$
...
We follow Matthew Emerton: \href{https://mathoverflow.net/q/12322/461}{https://mathoverflow.net/q/12322/461}%\medskip\centerline{\href{https://mathoverflow.net/q/12322/461}{https://mathoverflow.net/q/12322/461}}
...
To prove that $E\in\mathcal F(X)$ implies $(\star)$, %[the ``only if'' part of Exercise 7.21], 
...
To prove that $(\star)$ implies $E\in\mathcal F(X)$, %[the ``if'' part] 
...
Then $\Sigma$ is nonempty because %it contains 
...
\end{equation}
To prove \eqref{oex0}, assume by contradiction that we have $\overline{E\cap X_0}<X_0$. %, and let us derive a contradiction as follows. 
...
\textbf{Solution.} If $E$ is open in $X$, then the indicated condition holds because $E\cap X_0\ne\varnothing$ implies that $E\cap X_0$ is a non-empty open subset of $X_0$ contained in $E\cap X_0$. %: straightforward (details left to the reader).
...
Proof of (a): %The subset $X_0$ is irreducible, for t
...
The fact that ``$E$ is quasi-compact'' in the hint follows from Exercise 6.6 p.~79 of the book. The phrase ``where $X_0=\Spec(A/\ppp)$'' follows from Section~\ref{irr} p.~\pageref{irr} above. %For the last step of the argument, see Section~\ref{522} p.~\pageref{522} above. 
...
\end{equation}%Indeed, if $A$ is Artin, then, as observed before Proposition 8.6, $\mmm$ is the nilradical of $A$, and Proposition 8.5 p.~89 implies that $\mmm$ is nilpotent. 
...
To prove (i) $\implies$ (iii) on can use \eqref{fiber2} p.~\pageref{fiber2} above and the following Lemma:
...
%Hint: Note that for $0\ne a\in A$ there is a unique $n\in\mathbb N$ such that $a\in\mmm^n\setminus\mmm^{n+1}$, and set $n(a):=n$. Prove that there a unique valuation $v:K^*\epi\mathbb Z$ such $v(a)=n(a)$ for all $a\in A\setminus(0)$ (here $K$ is the field of fractions of $A$), and show that $A$ is the discrete valuation ring attached to $v$. 

%\subsection{Page 99, Exercise 9.5}%

%\textbf{Statement.} Let $M$ be a finitely-generated module over a Dedekind domain. Prove that $M$ is flat if and only if $M$ is torsion-free.

%\textbf{Hint.} See Exercises 3.13 p.~45 and 7.16 p.~86 of the book. %Also note that a module is flat if its finitely generated submodules are flat. 
...
\end{equation}
%Assume by contradiction that $G\mapsto G^p$ is right exact. This functor being left exact, it is exact. 
...
$$ 
%As we are assuming that $G\mapsto\widehat G$ is exact, the sequence $$0\to A^p\xr{\alpha}B^p\xr{\pi}B^p\to 0.$$ is also exact. This implies $A^i\simeq A^p$. But w
We have 
$$
...
%hence $\overline A\not\simeq\widehat A$, contradiction.
...
$\bu$ first equivalence: obvious, %Krull's Theorem,
...
$$ 
%By Krull's Theorem we have $\widehat M=0$ if and only if $M=\aaa M$. Finally, by Corollary~2.5 p.~21 of the book we have $M=\aaa M$ if and only if there is an $a\in\aaa$ such that $x=ax$ for all $x\in M$. 
...
\end{split} 
\] 
As $\deg b<r$, the polynomial $g_{k+1}$ is monic of degree $r$. The inequality $\deg a\le n-r$ (see \eqref{dega}) implies $\deg h_{k+1}\le n-r$.%, and the condition $$\overline{h_{k+1}}=\overline h\text{ with }\overline h\text{ monic}$$ yields $\deg h_{k+1}=n-r$.
\end{proof}
...
\end{array}
$$ 
(The condition that the roots $\alpha,a,a_0$ and $y(x)$ are simple is implicit.)%Set $A:=k[[x]]$, $\aaa:=(x)$ in Theorem~\ref{hl} p.~\pageref{hl}. 
...
\textbf{Solution.} Using Proposition 10.14 p.~109 of the book it is easy to see that $B$ is $A$-flat. Let $\phi:A\to B$ be the inclusion, $\psi:B\to A$ the evaluation at $0$, and $\mmm$ a maximal ideal of $A$. It suffices to show that the extension of $\mmm$ along $\phi$ is a proper ideal of $B$ (see Exercise 3.16iii p.~45 of the book). If this extension was the unit ideal of $B$, then the extension of $\mmm$ along $\psi\circ\phi$ would be the unit ideal of $A$. But this latter extension is $\mmm$ because $\psi\circ\phi$ is the identity of $A$.%define the $A$-algebra surjective morphism $\phi:B\to A/\mmm$ as the composite of $B\to A$, $f\mapsto f(0)$ and the canonical projection $A\to A/\mmm$. %, and let $\nnn$ be the kernel of $\phi$. 
...
But this follows from that fact that $\frac{\partial f}{\partial X_i}(0)$ is the coefficient of $X_i$ in $f$. %It is clear that $f\not\in\mmm^2$ if $\frac{\partial f}{\partial X_i}(0)\ne0$ for some $i$. To prove the converse, note that $\frac{\partial f}{\partial X_i}(0)$ is the coefficient of $X_i$ in $f$. 
...
\textbf{Solution.} Let $\qqq$ be the $\mmm$-primary ideal $(x_1,\ldots,x_d)$, %and note that $\qqq$ defines the same topology on $A$ as $\mmm$ (see proof of Proposition 11.6 p.~119). 
...
Proof of (d). For any $f\in A$ write $M(f)$ for the set of all monomial occurring (with a nonzero coefficient) in $f$. Assume by contradiction that the ideal $\aaa$ of $A$ is contained in the union of the $\ppp_i$, but is contained in no $\ppp_i$. In particular $\aaa\ne(0)$. 

Let $0\ne f\in\aaa$ and let $I$ be the set of all $i\in\mathbb N$ such that $\mu\in\ppp_i$ for some $\mu\in M(f)$ (in other words $I$ is the set of all $i$ such that the indeterminate $x_i$ occurs in $f$). 

In particular $I$ is finite and $f\notin\ppp_j$ for any $j\notin I$. By Proposition 1.11i p.~8 of the book (see Proposition~\ref{111i} p.~\pageref{111i}), there is a $g\in\aaa$ which is not in $\ppp_i$ for any $i\in I$. 

Let $j\in\mathbb N$ be such that $j\notin N_i$ (or equivalently $x_j\notin\ppp_i$) for any $i\in I$. In particular the indeterminate $x_j$ does not occur in $f$, and the polynomials $f$ and $x_jg$ have no monomials in common. 

Clearly $f+x_jg\in\aaa$. Let $k$ be in $\mathbb N$. It suffices to show $f+x_jg\notin\ppp_k$. Equivalently it suffices to show that $f$ is not in $\ppp_k$ or that 
$$
x_j\text{ and }g\text{ are not in }\ppp_k.
$$ 
But if $f\in\ppp_k$ then $k\in I$, and thus $x_j$ and $g$ are not in $\ppp_k$. This completes the proof of (d).
...
$$ 
then $W(f+x_jg)\subset W(f)\setminus\{i\}$. % old Claim 2

\begin{proof}[Proof of (f)]
%Note~\ref{monid} implies that
As no monomial can occur simultaneously in $f$ and in $x_jg$, we have, by Note~\ref{monid}, 
$$
...
$$ 
Finally note that $x_jg\notin\ppp_i$, i.e. $i\notin W(x_jg)$.
%$f+x_jg\in\ppp_k$ if and only if $f$ and $x_jg$ are both in $\ppp_k$, or, in other words, $W(f+x_jg)=W(f)\cap W(x_jg)$, and in particular $W(f+x_jg)\subset W(f)$. 
%Note also that n
%Neither $x_j$ nor $g$ is in $\ppp_i$, hence $x_jg$ is not in $\ppp_i$, hence $f+x_jg$ is not in $\ppp_i$ (by Note~\ref{monid} and the fact that no monomial can occur simultaneously in $f$ and in $x_jg$), that is $i\notin W(f+x_jg)$.
\end{proof}
...
%\begin{note}\label{116b}Let $f:A\to A[x]$ be the natural embedding, let $$f^*:\Spec(A[x])\to\Spec(A)$$ be the continuous map induced by $f$, and let $\ppp$ be a prime ideal of $A$. Then we have $$f^{*-1}(\ppp)=\{\ \ppp[x]\ ,\ \ppp[x]+(x)\ \}.$$
%%then the fiber of $$f^*:\Spec(A[x])\to\Spec(A)$$ above a prime ideal $\ppp$ of $A$ is order isomorphic to the spectrum of $$k\otimes_AA[x]\simeq k[x],$$ where $k$ is the residue field at $\ppp$.
%\end{note}

%See Section~\ref{321iv} p.~\pageref{321iv} and Section~\ref{5547} p.~\pageref{5547} above.
%%For Exercise 3.21iv p.~47 of the book, see Section~\ref{321iv} p.~\pageref{321iv} above. For Exercise 4.7 p.~55 of the book, see Section~\ref{5547} p.~\pageref{5547} above. 
...
``Let $A$ be a Noetherian local ring of dimension $d$. Then there exists an $\mmm$-primary ideal in $A$ generated by $d$ elements $x_1,\ldots,x_d$.'' %Second sentence of the hint: see Proposition 11.13 p.~121 of the book. 
