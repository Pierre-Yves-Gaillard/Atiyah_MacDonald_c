...
We denote the height of the prime ideal $\mf p$ by $h(\mf p)$. We prove 
$$
...
If $\mf q^{\oo c}\underset\ne\subset\mf p^{\oo c}$, we have $h(\mf q^{\oo c})<h(\mf p^{\oo c})$, and the induction hypothesis implies $h(\mf q)\le 1+h(\mf q^{\oo c})\le h(\mf p^{\oo c})$.%, and it suffices to verify $h(\mf q^{\oo c})<h(\mf p^{\oo c})$, which is easy.

If $\mf q^{\oo c}=\mf p^{\oo c}$, we have $\mf q=\mf p^{\oo c}[x]$ by ($\star$). This implies $h(\mf q)=h(\mf p^{\oo c})$.
