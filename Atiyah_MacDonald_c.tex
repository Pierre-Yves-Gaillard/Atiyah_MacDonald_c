...
\subsection{Page 99, Exercise 9.6}% previous version https://docs.google.com/document/d/15qlLg96SBKJG55fxaZfevFdjzQIziYUDSj0OHYj7TGA/edit?folder=0ADbJLwZkWgt8Uk9PVA
...
We claim that the natural morphisms $A/\ppp^n\xrightarrow\alpha(A/\ppp^n)_\ppp\xleftarrow\beta A_\ppp/\ppp_\ppp^n$ are isomorphisms. 

Let $\pi:A\to A/\ppp^n$ be the canonical projection, set $S:=A\setminus\ppp$, and note that $\pi(s)$ is a unit for all $s\in S$ (because $A/\ppp^n$ is a local ring with maximal ideal $\pi(\ppp)$). We leave it to the reader to check that the formula $\alpha'(a/s)=\pi(s)^{-1}a$ (for $a\in A/\ppp^n$ and $s\in S$) defines a morphism $\alpha':(A/\ppp^n)_\ppp\to A/\ppp^n$, and that this morphism is inverse to $\alpha$. This proves the claim. 

As a result an $A$-module annihilated by $\ppp^n$ is "the same thing as" an $A_\ppp$-module annihilated by $\ppp_\ppp^n$. (We leave it to the reader to give a precise meaning to the previous sentence.) The same holds for \emph{finitely generated} modules. 
