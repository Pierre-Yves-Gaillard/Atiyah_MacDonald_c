...
Hint: For any abelian group $G$ write $G^p$ for the $p$-adic completion of $G$. Assume by contradiction that $G\mapsto G^p$ is right exact. This functor being left exact, it is exact. Set $C_j:=\mathbb Z/(p^j)$ for $j\le1$ and note the exact sequence 
$$
0\to C_1\xr{\alpha_j}C_j\xr{\pi_j}C_{j-1}\to 0,
$$ 
where $\pi_j$ is the multiplication by $p$ (we use the isomorphism $pC_j\simeq C_{j-1}$). This induces an exact sequence 
$$
0\to A\xr{\alpha}B\xr{\pi}B\to 0.
$$ 
Write $A^i$ for the completion of $A$ with respect to the filtration induced by the $p$-adic filtration of $B$. We leave it to the reader to check that applying Corollary 10.3 p.~104 of the book yields the exact sequence 
$$
0\to A^i\xr{\alpha}B^p\xr{\pi}B^p\to 0.
$$ 
As we are assuming that $G\mapsto G^p$ is exact, the sequence 
$$
0\to A^p\xr{\alpha}B^p\xr{\pi}B^p\to 0.
$$ 
is also exact. This implies $A^i\simeq A^p$. But we have $A^i\simeq\prod_{j\in\mathbb N}C_1$ %C_1^{\mathbb N}$ 
and $A^p\simeq A$, hence $A^i\not\simeq A^p$ (details again left to the reader), contradiction.
...
\textbf{Solution.} Using Proposition 10.14 p.~109 of the book it is easy to see that $B$ is $A$-flat. Let $\mmm$ be a maximal ideal of $A$, define the $A$-algebra surjective morphism $\phi:B\to A/\mmm$ as the composite of $B\to A$, $f\mapsto f(0)$ and the canonical projection $A\to A/\mmm$. %, and let $\nnn$ be the kernel of $\phi$. 
...
$$ 
and let $\aaa$ be an ideal of $A$ contained in the union of the $\ppp_i$. %Claim 1: We have $\aaa\subset\ppp_i$ for some $i$. 
We claim that 
...
$\bullet$ if $\ppp_0^{\oo c}=\ppp_1^{\oo c}$ then $\ppp_0=\ppp_0^{\oo c}[x]$ %, $\ppp_1=\ppp_0+(x)$ 
and $\ppp_1^{\oo c}<\ppp_2^{\oo c}$,

$\bullet$ if $\ppp_1^{\oo c}=\ppp_2^{\oo c}$ then $\ppp_1=\ppp_1^{\oo c}[x]$ %, $\ppp_2=\ppp_1+(x)$ 
...
The above chain contracting to a chain of length at most $n$ in $A$, there are indices $i$ such that $\ppp_i^{\oo c}=\ppp_{i+1}^{\oo c}$. Let $i$ be the largest such index. We have $\ppp_i=\ppp_i^{\oo c}[x]$ %and $\ppp_{i+1}=\ppp_i+(x)$ 
by Lemma~\ref{fiberx} p.~\pageref{fiberx} above, % the proof of the inequality $\dim A[x]\le1+2\dim A$ in Section~\ref{116} p.~\pageref{116}, %Note~\ref{116b} p.~\pageref{116b}, 
and 
\begin{equation}\label{hppp}
h(\ppp_i^{\oo c})=h(\ppp_i^{\oo c}[x])=h(\ppp_i)\ge i,
\end{equation}
the inequality following from Note~\ref{116a} p.~\pageref{116a}. %Using again Note~\ref{116a}
Then we get 
$$
