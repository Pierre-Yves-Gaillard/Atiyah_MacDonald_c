...
$$ 
is a chain of prime ideals of $A$, then 
$$
\mf p_0[x]\underset\ne\subset\dots\underset\ne\subset\mf p_n[x]\underset\ne\subset(\mf p,x)
$$ 
is chain of prime ideals of $A[x].\ \square$

\emph{Proof of the inequality} $\dim A[x]\le1+2\dim A$: For any chain $\mf p_0\underset\ne\subset\mf p_1\underset\ne\subset\mf p_2$ of prime ideals in $A[x]$ we have:
...
Last sentence of the hint: We denote the height of the prime ideal $\mf p$ by $h(\mf p)$.

We prove $h(\mf p)\le1+h(\mf p^{\oo c})$ for all prime ideal $\mf p$ of $A[x]$ by induction on $h(\mf p^{\oo c})$.

This will imply $\dim A[x]\le1+\dim A$, as desired.

If $h(\mf p^{\oo c})=0$ the statement follows from the argument used to solve Exercise 11.6 p.~126 of the book.

Assume $h(\mf p^{\oo c})>0$. %Let $\mf q$ be a prime ideal of $A[x]$ strictly smaller than $\mf p$. 

It suffices to show $h(\mf q)\le h(\mf p^{\oo c})$ for all prime ideal $\mf q$ of $A[x]$ strictly smaller than $\mf p$.

%As we have $h(\mf p)\le1+h(\mf q)$, it suffices to show $h(\mf q)\le h(\mf p^{\oo c})$.

Let $\mf q$ be such a prime ideal.
...
If $\mf q^{\oo c}=\mf p^{\oo c}$, we have $\mf q=\mf p^{\oo c}[x]$ by the argument used to solve Exercise 11.6 p.~126 of the book. This implies $h(\mf q)=h(\mf p^{\oo c})$.
