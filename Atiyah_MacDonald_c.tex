...
\section{P. 69, Exercise 16}\label{nnl}%
...
%The following is taken from Wikipedia \href{https://en.wikipedia.org/wiki/Noether_normalization_lemma}{\small https://en.wikipedia.org/wiki/Noether\_normalization\_lemma}
...
%Here are two versions of the proof. The second version is more detailed, but less concise.
\begin{proof} 
Let $x_1,\dots,x_n\in A$. Assume that $A=k[x_1,\dots,x_n]$, and that there is a non-constant polynomial $f\in k[X_1,\dots,X_n]$, where the $X_i$ are indeterminates, such that $f(\,x_1,\dots,x_n)=0$. Let $r$ be an integer larger than the degree of $f$. In view of $(\star)$ above, it suffices to show that $A$ is integral over 
$$
B:=k\left[x_2-x_1^r,\dots,x_n-x_1^{r^{n-1}}\right].
$$ 
As $A=B[x_1]$, it is enough to prove that $x_1$ is integral over $B$. In fact, we only need to check that 

$(\star\star)$ there is an $a\in k^\times$, an integer $d\ge1$, and a 
$$
g\in\bigg(k\left[X_2-X_1^r,\dots,X_n-X_1^{r^{n-1}}\right]\bigg)[X_1]
$$ 
of degree less than $d$, such that $f=aX_1^d+g$. 

But this is straightforward. 

More precisely, the key fact used to prove $(\star\star)$ is this: For any monomial $u\in k[X_1,\dots,X_n]$ occurring in $f$ (with a nonzero coefficient), let's denote by $u'$ the monomial $u$ \emph{viewed as a polynomial in the ring} 
$$
\bigg(k\left[X_2-X_1^r,\dots,X_n-X_1^{r^{n-1}}\right]\bigg)[X_1].
$$ 
Then $u'$ is monic. Moreover, if $u$ and $v$ are two distinct such monomials, then we have $\deg(u')\ne\deg(v')$.

In fact, if $u=X_1^{\alpha_1}\cdots X_n^{\alpha_n}$, then $u'$ is monic of degree $$\alpha_1+\alpha_2r+\cdots+\alpha_nr^{n-1}.$$
\end{proof}
% https://docs.google.com/document/d/1oTZtahmZAPeNJrbs65b2IzgWHBKkWBjzasbWtsHpWDA/edit Here is the second version of the proof:

[Added later: An approach which I find better is expounded in 

Mel Hochster, Noether normalization and Hilbert’s Nullstellensatz, 

\href{http://www.math.lsa.umich.edu/~hochster/615W10/supNoeth.pdf}{http://www.math.lsa.umich.edu/$\sim$hochster/615W10/supNoeth.pdf}\ ]
...
The assumption that $B$ is a domain (instead of an arbitrary ring) is not used. For another approach, see the following reference (already given in Section~\ref{nnl} p.~\pageref{nnl}:
