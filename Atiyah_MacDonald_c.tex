...
If we had $(B,g)<(C,h)$ for some $(C,h)\in\Sigma$, we would get $B=(\Ker g)B$ by Lemma~\ref{vrok}, and $(\Ker g)B\subset\Ker h$ by assumption. %and let $c$ be in $C\setminus B$. Then $c^{-1}$ is a non-unit of $B$. %Lemma 5.19 p.~65 of the book implies that
...
Hint: By Proposition 5.18 (i) and (ii), %$A$ and 
$B$ is %are 
local. % valuation rings of $K$. 
Let %$\mmm\subset A$ and 
$\nnn\subset B$ be the maximal ideal and set $\ppp:=A\cap\nnn$. Then %$\ppp\subset\mmm$, that is, $\ppp$ is a prime ideal of $A$, and 
...
Let $A$ be a ring, let $p,q$ be coprime monic polynomials in $A[x]$ with $\deg p=r$, and let $c$ be in $A[x]$. Then there are unique $a,b\in A[x]$ with $c=ap+bq$ and $\deg b<r$. 
\end{lem} 
\begin{proof}
The element $\bar q\in A[x]/(p)$ being a unit since $(p,q)=(1)$ in $A[x]$, there is a $b\in A[x]$ of least degree such that $\bar b\bar q=\bar c$ in $A[x]/(p)$. Since $p$ is monic of degree $r$, the elements $\bar 1,\bar x,\dots,\bar x^{r-1}$ freely generate $A[x]/(p)$ as a $A$-module, so $b$ is unique and of degree less than $r$. The polynomial $p$, being monic, is not a zero-divisor, so there is a unique $a\in A[x]$ such that $ap=c-bq$.
\end{proof}
...
$$
g_j\text{ monic of degree }r,\quad\deg h_j=n-r,\quad\overline{h_j}=\overline h,%\text{ for all }j\ge2,  
$$ 
$$
...
\end{split} 
\] 
As $\deg b<r$, the polynomial $g_{k+1}$ is monic of degree $r$. The inequality $\deg a\le n-r$ (see \eqref{dega}) implies $\deg h_{k+1}\le n-r$, and the condition 
$$
...
To prove (f) note that Remark~\ref{monid} implies that no monomial can occur simultaneously in $f$ and in $x_jg$, and thus $f+x_jag\in\ppp_k$ if and only if $f$ and $x_jg$ are both in $\ppp_k$, or, in other words, $W(f+x_jg)=W(f)\cap W(x_jg)$, and thus $W(f+x_jg)\subset W(f)$. Note also that neither $x_j$ nor $g$ is in $\ppp_i$, hence $x_jg\notin\ppp_i$, hence $f+x_jg\notin\ppp_i$ (by Remark~\ref{monid} and the fact that no monomial can occur simultaneously in $f$ and in $x_jg$), that is $i\notin W(f+x_jg)$.
...
$\bullet$ if $\ppp_0^{\oo c}=\ppp_1^{\oo c}$ then $\ppp_0=\ppp_0^{\oo c}[x]$, $\ppp_1=\ppp_0+(x)$ and $\ppp_1^{\oo c}<\ppp_2^{\oo c}$,

$\bullet$ if $\ppp_1^{\oo c}=\ppp_2^{\oo c}$ then $\ppp_1=\ppp_1^{\oo c}[x]$, $\ppp_2=\ppp_1+(x)$ and $\ppp_0^{\oo c}<\ppp_1^{\oo c}$.
...
$$ 
Here is the hint given in the book:

Let $\ppp$ be a prime ideal of height $m$ in $A$. Then there exist $a_1,\ldots,a_m\in\ppp$ such that $\ppp$ is a minimal prime ideal belonging to the ideal $\aaa=(a_1,\ldots,a_m)$. By Exercise 7 of Chapter 4, the ideal $\ppp[x]$ is a minimal prime ideal of $\aaa[x]$ and therefore the height $\ppp[x]$ is $\le m$. On the other hand, a chain of prime ideals 
$$
...
%(We shall use statements Note~\ref{116a} p.~\pageref{116a} and Note~\ref{116b} p.~\pageref{116b} above.)
...
The above chain contracting to a chain of length at most $n$ in $A$, there are indices $i$ such that $\ppp_i^{\oo c}=\ppp_{i+1}^{\oo c}$. Let $i$ be the largest such index. We have $\ppp_i=\ppp_i^{\oo c}[x]$ and $\ppp_{i+1}=\ppp_i+(x)$ by the proof of the inequality $\dim A[x]\le1+2\dim A$ in Section~\ref{116} p.~\pageref{116}, %Note~\ref{116b} p.~\pageref{116b}, 
and 
...
$$ 
the inequalities following respectively from Note~\ref{116a} and Display~\eqref{hppp}. This contradicts the definition of $n$. %which is impossible. 
$\square$
