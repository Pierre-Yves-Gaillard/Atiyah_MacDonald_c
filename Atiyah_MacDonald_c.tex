...
$\bu$ \textbf{Proposition 1.11i, Prime Avoidance}\label{111i}%
...
(d) $\mf m=\mf m^{\oo c}+(x)$. Proof: Let $f\in A[[x]]$ with $f=a_0+a_1x+\cdots$. There is a unique $g\in A[[x]]$ such that $f=a_0+xg$, and (a) implies that $f\in\mmm$ if and only if $a_0\in\mf m^{\oo c}$. %then $f\in\mmm$ by (a). $f=a_0+xg\in\mf m$, then $a_0\in\mf m^{\oo c}$ by (a).
...
\subsection{Page 13, Exercise 1.19}\label{119}%

The statement implies that the closed irreducible subsets of $\Spec(A)$ are the closures of the singletons, i.e. the $V(\ppp)$. 
...
\subsection{Page 13, Exercise 1.21v}%\label{hbt}
...
%\emph{If the natural morphism $M\otimes\aaa\to M\otimes A$ is injective for all finitely generated ideal $\aaa$, then $M$ is flat.}

%Let us state this in a more explicit way:
...
An absolutely flat local ring is a field: This follows immediately from Property~(iv) above (Section~\ref{227}) and Exercise 1.12 p.~11 of the book (see Section~\ref{112} p.~\pageref{112} above). %Let $(A,\mf m)$ be our local ring and let $\mu$ be in $\mf m$. It suffices to show $\mu=0$. If we had $\mu\ne0$, we would get $(\mu)=(e)$ for some nonzero idempotent $e$ (see end of Section \ref{227} above). Then $e\in\mf m=\mf R$ would imply that $f:=1-e$ is a unit, hence $f=f^2$ entails $f=1$ and thus $e=0$, a contradiction. 
...
(A_\ppp/\ppp_\ppp)\otimes_AB\simeq B_\ppp/\ppp B_\ppp\simeq(B/\ppp B)_\ppp=\big(B/\ppp^{\oo e}\big)_\ppp.%\simeq B_\ppp/\ppp B_\ppp.
\end{equation} 
Proof of Claim~1. The second isomorphism follows from the exactness of localization and the equality $(\ppp B)_\ppp=\ppp B_\ppp$. %is clear. 
...
$$ 
are exact, and that the image of $\ppp_\ppp\otimes_AB\to B_\ppp$ is $\ppp B_\ppp$. This proves the first isomorphism of \eqref{fiber2}, completting the proof of Claim~1. 
%$$(A_\ppp/\ppp_\ppp)\otimes_AB\simeq(A/\ppp)_\ppp\otimes_AB\simeq(A/\ppp)_\ppp\otimes_{A/\ppp}A/\ppp\otimes_AB$$ $$\simeq(A/\ppp)_\ppp\otimes_{A/\ppp}B/\ppp B\simeq(B/\ppp B)_\ppp=\big(B/\ppp^{\oo e}\big)_\ppp,$$ 
...
A multiplicatively closed subset $S$ of a ring $A$ is said to be \emph{saturated} if $xy\in S\iff x\in S$ and $y\in S$. %XY E S ~ X E Sand y E S.

Prove that

(i) $S$ is saturated $\iff A\setminus S$ %~ A S 
...
$$ 
which implies $ax=0$ because the map $M\to Aa^{-1}\otimes_AM$, $y\mapsto a^{-1}\otimes ay$ is an isomorphism. %If $1\otimes x=0$ in $K\otimes M$, then $1\otimes x=0$ in $A\frac1a\otimes M$. This implies $ax=0$.
...
$$ 
proved in \eqref{fiber2} p.~\pageref{fiber2}. %Section~\ref{res} p.~\pageref{res}.
...
\subsection{Page 48, Exercise 3.27iv}%

See Section~\ref{223} p.~\pageref{223}.
...
To complete the proof of Claim~1 set $T:=\{\ a^ns|\ n\in\mathbb N,s\in A\setminus\ppp\}$. Claim~2 implies that $T$ is disjoint from $(0:b)$. Thus there is a prime ideal $\qqq$ which contains $(0:b)$ and is disjoint from $T$, and we have $A\setminus\ppp\subset T\subset A\setminus\qqq$, that is $(0:b)\subset\qqq\subset\ppp$. By minimality of $\ppp$ over $(0:b)$ this forces $\qqq=\ppp$, and thus $a\in\ppp\cap T=\qqq\cap T=\varnothing$, a contradiction. This completes the proof of Claim~1. $\square$ 

$\bu$ \textbf{Proof of the inclusion} 
\begin{equation}\label{da}
D(S^{-1}A)\subset D(A)\cap\Sp(S^{-1}A):
\end{equation}
To start the proof of \eqref{da}, let $\ppp$ be a prime ideal of $A$ disjoint from $S$ such that $S^{-1}\ppp\in D(S^{-1}A)$. It suffices to show $\ppp\in D(A)$. Note that $S^{-1}\ppp$ is minimal over $(0:\frac a1)$ for some $a\in A$. % and some $s\in S$ such that $\frac as$ is a zero-divisor. 
It suffices to show that %$a$ is a zero-divisor and that 
\begin{equation}\label{da2}
\ppp\text{ is minimal over }(0:a).
\end{equation}
%It is easy to prove that $a$ is a zero-divisor. It remains to verify that $\ppp$ is minimal over $(0:a)$. 
...
To complete the proof of \eqref{da2}, assume by contradiction that there is a prime ideal $\qqq$ such that 
$$
(0:a)\subset\qqq<\ppp.
$$ 
\textbf{Claim 4:} $(0:\frac a1)\subset S^{-1}\qqq$.

\emph{Proof of Claim 4.} Assuming $\frac bs\frac a1=0$ (obvious notation), we get $abt$ for some $t\in S$, hence $bt\in\qqq$, hence $b\in\qqq$, and thus $\frac bs\in S^{-1}\qqq$. $\square$ 

To complete the proof of \eqref{da2}, note that we get 
$$
\left(0:\frac a1\right)\subset S^{-1}\qqq<S^{-1}\ppp,
$$ 
contradicting the minimality of $S^{-1}\ppp$ over $(0:\frac a1)$. %$\ppp$ over $(0:a)$. 
$\square$
...
By Corollary~\ref{c45} p.~\pageref{c45} above (which is a corollary to the first uniqueness theorem, that is to Theorem 4.5 p.~52 of the book), it suffices to show that $(\aaa:x)$ is prime. If not there would be $y,z\in A$ such that 
$$
...
Then the product $xy$ is in $\mathfrak{q}_2\cap\cdots\cap\mathfrak{q}_n\subset\mathfrak{q}_1$. But this is impossible since $x$ is not in $\qqq_1$ and $y$ is not in $\ppp_1$. %Since $\mathfrak{q}_1$ is $\mathfrak{p}_1$-primary, so either $x\in \mathfrak{q}_1$ or $y\in \mathfrak{p}_1$. But by construction of $x$ and $y$, this is not the case.
...
$$ 
This yields $M=N+P$, so $r_M(N+P)=(1)$, but $r_M(N)=r_M(P)=0$. % Let $A\ne0$, $M=A\oplus A$ with action $a(b,c)=(ab,ac)$, $N=A\oplus(0)$, and $P=(0)\oplus A$. Then $M=N+P$, so $r_M(N+P)=(1)$, but $r_M(N)=r_M(P)=0$.

\subsection{Page 57, Exercise 4.21}%

Analogue of (4.3): Let $Q_1$ and $Q_2$ be $\ppp$-primary in $M$, and let us show that $Q_1\cap Q_2$ is also $\ppp$-primary in $M$. Let $a\in A$, $x\in M$, $x\notin Q_1\cap Q_2$ satisfy $ax\in Q_1\cap Q_2$. We can assume $x\notin Q_1$. We get $a\in\ppp$, and thus $a^nM\subset Q_1\cap Q_2$ for some $n$.
...
\end{matrix}
$$ 
Analogue of (4.4): For the analogues of (i) and (ii), replace $\qqq$ with $Q$ and $x\in A$ with $x\in M$. I see no analogue of (iii). For the proof of (ii) (using the same notation as above): 
$$
\begin{matrix}
y\in(\qqq:x)&xy\in\qqq&x\notin\qqq&\qqq\subset(\qqq:x)\subset\ppp&r(\qqq:x)=\ppp\\ \\ 
y\in(Q:x)&yx\in Q&x\notin Q&(Q:M)\subset(Q:x)\subset\ppp&r(Q:x)=\ppp
\end{matrix}
$$ 
Continuation of the table:
$$
\begin{matrix}
yz\in(\qqq:x)&xyz\in\qqq&xz\in\qqq&z\in(\qqq:x)\\ \\ 
yz\in(Q:x)&yzx\in Q&zx\in Q&z\in(Q:x)
\end{matrix}
$$ 
...
$$
\begin{matrix}
\aaa\subset A&\ppp\supset\aaa&\ppp\supset\aaa=\bigcap\qqq_i&r(\qqq_i)\\ \\ 
0\subset M&\ppp\supset(0:M)&\ppp\supset(0:M)=\bigcap(Q_i:M)&r(Q_i:M)
\end{matrix}
$$ 
(4.7): Replace 
...
$$ 
(ii) If $S\cap\ppp=\varnothing$,then $S^{-1}Q$ is $S^{-1}\ppp$S-primary and its contraction in $M$ is $Q$. Hence primary ideals correspond to primary ideals in the correspondence between submodules in $S^{-1}M$ and contracted submodules in $M$.

Let us show $r(S^{-1}Q:S^{-1}M)=S^{-1}\ppp$. It suffices to check $(S^{-1}Q:S^{-1}M)^{\text c}\subset (Q:M)$. Let $a$ be in $(S^{-1}Q:S^{-1}M)^{\text c}$, that is $a/1\in (S^{-1}Q:S^{-1}M)$, and let $x$ be in $M$. It suffices to verify $ax\in Q$. We have  
$$
...
$$
\begin{matrix}
\aaa\subset A&\qqq_i&r(\qqq_i)\\ \\ 
0\subset M&Q_i&r(Q_i:M)
\end{matrix}
$$ 
\subsection{Primary decomposition of a submodule after Bourbaki}%\label{310ii}%%
...
\subsection{Page 65, Theorem 5.20}\label{520}%

The proof of Theorem 5.20 shows:
...
\emph{In the setting of Section~\ref{520} above, let x be a non-zero element of K. If $\mmm[x]\ne B[x]$, then there is a maximal ideal $\mmm'$ of $B[x]$ containing $\mmm$ and $B/\mmm'$ is algebraic over $B/\mmm$. If $\mmm[x^{-1}]\ne B[x^{-1}]$, then there is a maximal ideal $\mmm'$ of $B[x^{-1}]$ containing $\mmm$ and $B/\mmm'$ is algebraic over $B/\mmm$.}
...
As 
$$
\sigma(\qqq)\cap A^G=\sigma\left(\qqq\cap\sigma^{-1}(A^G)\right)=\sigma\left(\qqq\cap A^G\right)=\sigma(\ppp)=\ppp,
$$ 
...
%\textbf{Noether Normalization Theorem}\subsubsection{Noether Normalization Theorem}%Before proceeding, recall the following fact (see Remark p.~60 of the book): %it might be useful to spell out a fact which is proved but, it seems to me, not stated explicitly in the book:%$(\star)$ 
...
\subsection{Page 71, Exercise 5.26}%\label{nnt}%
...
$\bullet\ U\cap\overline{U\cap C}\subset U\cap C$ because $\overline{U\cap C}\subset\overline C=C$. $\square$ %(because $U\cap C\subset C$ and $C$ is closed). \\ If $S\subset X$ is open in its closure $\overline S$, we have $S=U\cap\overline S$ for some open subset $U$. \\ Conversely, if $S=U\cap C$ with $U$ open and $C$ closed, then $$S=U\cap C=U\cap\overline{U\cap C}.$$ To prove this it suffices to show $U\cap\overline{U\cap C}\subset U\cap C$. Let $u$ be in $U\cap\overline{U\cap C}$. If $u$ was not in $C$, then $u$ would be in $V:=X\setminus C$, and $V$ would meet $U\cap C$, contradiction. \\ Hint for the first part: prove (2) $\iff$ (1)$\iff$ (3).

Part 1. Condition (3) is clearly equivalent to 
...
Part 2. Here is a hint for the proof of (i) $\iff$ (ii). Let $M\subset\Spec(A)$ be the set of maximal ideals. It suffices to show that (a) and (b) below are equivalent.
...
Let us prove (i) $\iff$ (iii) in the second part of the exercise. It suffices to show that (c) and (d) below are equivalent.
...
\subsection{Page 72, Exercise 5.27}%\label{nnt}%

Let $\Gamma$ be the set denoted by $\Sigma$ on page 65 of the book (see Section~\ref{65} p.~\pageref{65}), and let $(B,g)$ be in $\Gamma$. By Lemma 5.19 p.~65 of the book, $B$ is in $\Sigma$. 

By Theorem 5.21 p.~66 of the book (see Section~\ref{521} p.~\pageref{521}), it suffices to show that $(B,g)$ is maximal in $\Gamma$ if and only if $B$ is maximal in $\Sigma$. 

Assume that $B$ is not maximal in $\Sigma$, so that there is a $C\in\Sigma$ with $B<C$. Let $c$ be in $C\setminus B$. We have $B<B[c]=:B'\subset C$, and the proof of Theorem 5.21 p.~66 of the book (see Section~\ref{521p} p.~\pageref{521p}), $(B,g)$ is not maximal in $\Gamma$. 

Assume that $(B,g)$ is not maximal in $\Gamma$, so that there is a $(C,h)\in\Gamma$ with $(B,g)<(C,h)$. By Theorem 5.21 p.~66 we can assume that $C$ is local. Leting $\mmm\subset B$ and $\mf n\subset C$ be the maximal ideals, we get 
$$
\mmm=\Ker f=C\cap\Ker g=C\cap\mf n,
$$ 
and thus $B<C$ in $\Sigma$.

\subsection{Page 72, Exercise 5.28}%\label{nnt}%
...
\subsection{Page 72, Exercise 5.29}%\label{nnt}%

Hint: By Proposition 5.18 (i) and (ii), $A$ and $B$ are local valuation rings of $K$. Let $\mmm\subset A$ and $\ppp\subset A$ be the maximal ideals. Then $\ppp\subset\mmm$, that is, $\ppp$ is a prime ideal of $A$, and we have $A_\ppp=B$. 

\subsection{Page 72, Exercise 5.30}%\label{nnt}%
...
\subsection{Page 72, About Exercise 5.31}%\label{hbt}
...
\subsection{Page 72, Exercise 5.32}%\label{nnt}%
...
\subsection{Page 78, Exercise 6.3}%\label{nnt}%
...
\subsection{Page 78, Exercise 6.4}%\label{nnt}%

(a) Hint: use the above generalization.

(b) See Example 3 p.~74 of the book. 

\subsection{Page 79, Exercise 6.7}%\label{nnt}%
...
\subsection{Page 79, Exercise 6.8}%\label{nnt}%
...
\subsection{Page 79, Exercise 6.9}%\label{nnt}%

See Exercise 1.20iv p.~13 of the book.

\subsection{Page 79, Exercise 6.10}%\label{nnt}%
...
(i) $\implies$ (ii): Use Lemma 4.5 p.~51 and Proposition 4.8 p.~53. (This implication holds even if $\aaa$ is reducible.)
...
$$ 
and thus $0=a^nz=y$. This completes the proof.% proves the claim. As $Ax\ne0$ and $0$ is irreducible, the claim implies that $a^nM=0$, and thus that $a$ is nilpotent in $M$, as required. 
...
$$
\begin{matrix}
\aaa\ne(1)&x\in A&\qqq_i&\ppp_i=r(\qqq_i)&\aaa_i&\ppp_i^m\subset\qqq_i&\aaa_i\cap\ppp_i^m\\ \\ 
N<M&x\in M&Q_i&\ppp_i=r(Q_i:M)&N_i&\ppp_i^mM\subset Q_i&N_i\cap\ppp_i^mM
\end{matrix}
$$ 
...
\end{lem} 
We summarize Lemma~\ref{719L3} by saying that ``we have replaced $U_1$ with $V_i$ in the equality $W=U_1\cap U_2$''. 
...
$$ 
and $p\ge1$ is minimum for this condition. It suffices to show $p=1$. We do this by applying Lemma~\ref{719L2} with $U:=U_1$, $V:=U_2$, $U':=U_2\cap V_{i_1}$, $V':=V_{i_2}\cap\cdots\cap V_{i_p}$. 
\end{proof}
...
$$ 
Then Part (b) of Proposition~\ref{719P1} entails that $f^{-1}(I)$ and $g^{-1}(I)$ are equipotent. In particular, if $\ppp\in P$ is minimal, $f^{-1}(\ppp)$ and $g^{-1}(\ppp)$ have same cardinality, and an obvious induction completes the proof.

\subsection{Page 87, Exercise 7.21}%

Let us denote by $\mathcal F(X)$ the set designated by $\mathcal F$ in Exercise 7.20. 
...
To prove the ``if'' part, we follow the hint: 

Set $\mathcal F:=\mathcal F(X)$ and let $\Sigma$ be the set of all closed subsets $X'$ of $X$ such that $E\cap X'\notin\mathcal F$. Then $\Sigma$ is nonempty because it contains $X$. Let $X_0$ be a minimal element of $\Sigma$. In particular 
\begin{equation}\label{721}
E\cap X_0\notin\mathcal F.
\end{equation} 
The subset $X_0$ is irreducible, for if we have $X_0=C\cup D$ with $C,D$ closed and $<X_0$, we would have $C,D\notin\Sigma$ by minimality of $X_0$, and thus $E\cap C$ and $E\cap D$ in $\mathcal F$, which would imply 
$$
...
Statement: Let $X$ be a Noetherian topological space and let $E$ be a subset of $X$. Show that $E$ is open in $X$ if and only if, for each irreducible closed subset $X_0$ in $X$, either $E\cap X_0=\varnothing$ or else $E\cap X_0$ contains a non-empty open subset of $X_0$.

Solution: If $E$ is open in $X$, then the indicated condition holds: straightforward (details left to the reader).
...
Assume $E\cap X_0=\varnothing$. This implies $E\cap X_0=X_0$ with $E\cap X_0$ not closed and $X_0$ closed, a contradiction.
...
The fact that ``$E$ is quasi-compact'' in the hint follows from Exercise 6.6 p.~79 of the book. The phrase ``where $X_0=V(\ppp)$'' follows from Section~\ref{119} p.~\pageref{119} above.

\subsection{Page 87, Exercise 7.24}% 

Let us spell out the last two sentences of the hint.

Let $\ppp_0$ be in $X$, and set $X_0:=V(\ppp_0)$. Assuming that $f$ has the going-down property, we want to show that $E:=f^*(Y)$ is open in $X$. Suppose $E\cap X_0\ne\varnothing$, and thus $\ppp_0\in E$ (by the going-down property). As $E$ is constructible by Exercise 7.23, it suffices, in view of Exercise 7.22, to show that $E\cap X_0$ contains a nonempty open subset of $X_0$, and Exercise 7.22 tells us that it is even enough to check the inclusion 
...
$$ 
we get $\aaa\subset\ppp_0$ because $\ppp_0\in E\cap X_0$, that is 
$$
...
It is proved on p.~94 of the book that DVR are noetherian. Exercise 5.18 p.~72 of the book says that the ideals of a valuation ring are totally ordered. In particular finitely generated ideals are principal. Thus any noetherian valuation ring $A$ is a principal ideal domain. Hence, if $A$ is not a field, then it is of dimension one, and is therefore a DVR by Proposition 9.2 p.~94 of the book. 

\subsection{Page 99, Exercise 9.4}% 

Hint: for $0\ne a\in A$ there is a unique $n\in\mathbb N$ such that $a\in\mmm^n\setminus\mmm^{n+1}$, and set $n(a):=n$. Prove that there a unique valuation $v:K^*\epi\mathbb Z$ such $v(a)=n(a)$ for all $a\in A\setminus(0)$, and show that $A$ is the DVR attached to $v$. 

%\subsection{Page 99, Exercise 9.5}% See Exercises 3.13 p.~45 and 7.16 p.~86 of the book. 
...
To prove this let us show that any nonempty open subset $U\subset A$ meets $\mmm$. Set $K:=A/\mmm$ and let $\pi:A\to K$ be the canonical projection. It suffices to check that $0\in\pi(U)$. But the $\aaa$-adic topology of $K$ being the coarse topology, $\pi(U)$ is the unique nonempty subset of $K$, that is $K$ itself. %Let $u$ be in $U$. There some $n\in\mathbb N$ such that $u+\aaa^n\subset U$, and it suffices to prove that $u+\aaa^n$ meets $\mmm$. Let $\alpha$ be in $\aaa\setminus\mmm$. If $x\in A$ is a solution to the equation $$\alpha^nx\equiv u\bmod\mmm,$$ then we have $u-\alpha^nx\in\mmm$, as desired. 

\subsection{Page 114, Exercise 10.7}% 

See Section~\ref{1019} p.~\pageref{1019} above. 

\subsection{Page 115, Exercise 10.9}% 

We follow Jeffrey Daniel Kasik Carlson. 

\begin{lem}\label{L106a} 
Let $B$ be a ring, let $p,q$ be are coprime monic polynomials in $B[x]$ with $\deg p=r$, and let $c$ be in $B[x]$. Then there are unique $a,b\in B[x]$ with $c=ap+bq$ and $\deg b<r$. 
\end{lem} 
\begin{proof}
The element $\bar q\in B[x]/(p)$ being a unit since $(p,q)=(1)$ in $B[x]$, there is a $b\in B[x]$ of least degree such that $\bar b\bar q=\bar c$ in $B[x]/(p)$. Since $p$ is monic of degree $r$, the elements $\bar 1,\bar x,\dots,\bar x^{r-1}$ freely generate $B[x]/(p)$ as a $B$-module, so $b$ is unique and of degree less that $r$. Now $c\equiv bq\bmod p$, and $p$, being monic, is not a zero-divisor, so $ap=c-bq$ has a unique solution $a\in B[x]$. 
\end{proof}
...
Now we solve Exercise 10.6. We need only assume that $A$ is complete with respect to some ideal $\mmm$; we do not necessarily need $\mmm$ maximal or $A$ local. To emphasize the fact that $\mmm$ is not necessarily maximal, we denote this ideal by $\aaa$. Recall that $A$ is complete for the $\aaa$-topology. 

Let $g_1,h_1$ be monic lifts of $\bar g,\bar h$ to $A[x]$ and note the we have 
$$
f-g_1h_1\equiv0\quad\bmod\aaa.
$$ 

We shall inductively construct $g_2,h_2,g_3,h_3,\dots\in A[x]$ with 
$$
g_j\text{ monic of degree }r%\text{ for all }j\ge2,  
$$ 
$$
f-g_jh_j\equiv0\quad\bmod\aaa^j%\text{ for all }j\ge2
$$ 
and 
$$
g_j\equiv g_{j-1},\quad h_j\equiv h_{j-1}\quad\bmod\aaa^{j-1}%\text{ for all }j\ge2
$$ 
for all $j\ge 2$. 
...
$$
g\equiv g_j,\quad h\equiv h_j\quad\bmod\aaa^j
$$ 
for all $j\ge1$. We will then have 
$$
f-gh=(f-g_jh_j)+(g_jh_j-gh)\equiv0\quad\bmod\aaa^j
$$ 
for all $j\le1$. Since $A$ is $\aaa$-adically complete, $\bigcap\aaa^j=0$, so $f=gh$. As $f$ and $g$ are monic, so is $h$. 

It only remains to construct the $g_j$ and $h_j$. 

Let $k$ be $\ge2$ and suppose inductively we have found $g_2,$ $h_2,$ $g_3,$ $h_3,$ $\dots,$ $g_k,$ $h_k$ such that 
$$
g_j\text{ is monic of degree }r,
$$ 
$$
f-g_jh_j=c\equiv0\quad\bmod\aaa^j
$$ 
for $1\le j\le k$, and 
$$
g_j\equiv g_{j-1},\quad h_j\equiv h_{j-1}\quad\bmod\aaa^{j-1}
$$ 
for $2\le j\le k$. 

Since by Proposition 10.15.iv p.~109 of the book, $\aaa^k\subset\mf R(A)$, it follows from Lemma~\ref{L106b} with $\bbb=\aaa^k$, that $(g_k,h_k)=(1)$ in $A[x]$. By Lemma~\ref{L106a} with 
$$
B=A,\quad p=g_k,\quad q=h_k,
$$ 
there are unique $a,b\in A[x]$ with $\deg b<r$ and $ag_k+bh_k=c$. Taking $B=A/\aaa^k$ in Lemma~\ref{L106a} and using uniqueness, we see that since $\bar c=0$ we also have $\bar a=\bar b=0$, or $a,b\in\aaa^k[x]$. Setting  
$$
...
&=c-c+ab=ab\in\aaa^{2k}[x]\subset\aaa^{k+1}[x].\\
\end{split} 
\] 
...
$$ 
be a partition of $\mathbb N$, let $K$ be a field, let $A$ be the $K$-algebra $K[x_0,x_1,\dots]$, where the $x_i$ are indeterminates, for each $i\in\mathbb N$ let $\ppp_i$ be the ideal of $A$ generated by the $x_j$ with $j\in N_i$, for $f\in A$ set 
$$
W(f):=\{i\in\mathbb N\ |\ f\in\ppp_i\},
$$ 
and let $\aaa$ be an ideal of $A$ contained in the union of the $\ppp_i$. 

Claim 1: We have $\aaa\subset\ppp_i$ for some $i$. 

Claim 2: If $f,g\in A$ and $i,j\in\mathbb N$ satisfy 
$$
g\notin\ppp_i,\quad j\notin N_i,\quad x_j\text{ doesn't occur in }f,
$$ 
then $W(f+x_jg)\subset W(f)\setminus\{i\}$. 

To prove Claim 2 note that no monomial can occur simultaneously in $f$ and in $x_jg$. 

Let us show that Claim 2 implies Claim 1. 

Assume by contradiction that for all $i$ we have $\aaa\not\subset\ppp_i$. Note that $W(f)$ is finite if $0\ne f\in A$. Denote by $|X|$ the cardinality of any set $X$. Let $f\in\aaa$ with $|W(f)|$ minimum. In particular $|W(f)|$ is a nonnegative integer. It is even a positive integer because $\aaa$ is contained in the union of the $\ppp_i$. Let $i$ be in $W(f)$ and $g$ be in $\aaa\setminus\ppp_i$. By Claim~2 there is a $j$ such that $f+x_jg$ is an element of $\aaa$ satisfying $|W(f+x_jg)|<|W(f)|$, a contradiction. 

\subsection{Page 126, Exercise 11.6}% previous version https://docs.google.com/document/d/1pXphjmJVG5y2JSIiE6W0ZBNaBsdVPPTXytsjLOLYc7g/edit
...
$$ 
\emph{above a prime ideal $\ppp$ of $A$ is order isomorphic to the spectrum of} 
$$
...
$\bullet$ Inequality $\oo{height }\ppp[x]\le m$: see Corollary 11.16 p.~121 of the book. Note that we have $\aaa=\sum_{i=1}^ma_iA$ and $\aaa[x]=\sum_{i=1}^ma_iA[x]$. 
...
The above chain contracting to a chain of length at most $n$ in $A$, there are indices $i$ such that $\ppp_i^{\oo c}=\ppp_{i+1}^{\oo c}$. Let $i$ be the largest such index. We have $\ppp_i=\ppp_i^{\oo c}[x]$ by ($\star\star$), and 
$$
h(\ppp_i^{\oo c})=h(\ppp_i^{\oo c}[x])=h(\ppp_i)\ge i,
$$ 
the inequality following from ($\star$). Using again ($\star$) we get  %and $\ppp_i=\ppp_i^{\oo c}[x]$ by ($\star$), and thus $h(\ppp_i^{\oo c})=h(\ppp_i)\ge i$. This implies
$$
