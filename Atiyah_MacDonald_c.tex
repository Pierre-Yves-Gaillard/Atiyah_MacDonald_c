...
Assume that $(A,f)$ is not maximal in $\Phi(K,\Omega)$, so that there is a $(B,g)\in\Phi(K,\Omega)$ with $(A,f)<(B,g)$. By Theorem 5.21 p.~66 (see Section~\ref{521} p.~\pageref{521}) we can assume that $B$ is local. Letting $\mmm\subset A$ and $\nnn\subset B$ be the maximal ideals, we get 
$$
\mmm=\Ker f=B\cap\Ker g=B\cap\nnn,
$$ 
