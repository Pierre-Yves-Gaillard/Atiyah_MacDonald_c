...
\end{tikzcd}
$$ 
%$$M=M_0>M_1>\cdots>M_{n+1}=0,\quad M=N_0>N_1>\cdots>N_{n+1}=0.$$ We must prove that the respective multisets of quotients are equal. 
...
%the multisets %$$S=\{[M_i/M_{i+1}]\}_{i=1}^n\text{ and }T=\{[N_i/N_{i+1}]\}_{i=1}^n$$ of quotients attached to the two series for $M_1$ are equal, hence the multisets of quotients attached to the two original series are also equal.%, since $M/M_1=M/N_1$, the quotient multisets of the two composition series for $M$ are equal. 

Case 2: $M_1\ne N_1$. Setting $P_2:=M_1\cap N_1$, %Note that $M_1<M_1+N_1\subset M$, so, since $M/M_1$ was assumed simple, $M_1+N_1=M$. Now $$M_1/P_2 = M_1/(M_1\cap N_1)\simeq(M_1+N_1)/N_1=M/N_1$$ by the second isomorphism theorem (Proposition 2.1.ii p.~19 of the book), and this quotient is simple. Symmetrically, $N_1/P_1\simeq M/M_1$. By the proof of Proposition 6.7 p.~77 of the book, $\ell(P_2)\le\ell(M_1)=-1n$ is finite, so $P_2$ has a composition series $P_2>P_3>\cdots>P_k=0$. Write $U$ for the quotient multiset. Then $M_1>P_1>\cdots>P_p=0$ is a composition series for $M_1$. Since $\ell(M_1)=n$, we have $p=n$, and by the induction hypothesis, the multiset $$\{[M_1/P_1]\}\cup U =\{[M/N_1]\}\cup U$$ is the same as the multiset $S=\{[M_i/M_{i+1}]\}_{i=1}^n$. Then the quotient multiset for the $M_i$ composition series of $M$ is $$\{[M/M_1]\}\cup S=\{[M/M_1],[M/N_1]\}\cup U.$$ Similarly $N_1>P_1>\cdots>P_p=0$ is a composition series for $N_1$ with quotient multiset $\{[N_1/P_1]\}\cup U=\{[M/M_1]\}\cup U$, by inductive assumption equal to the multiset $$T:=\{[N_i/N_{i+1}]\}_{i=1}^n.$$ Then the $N_i$ composition series for $M$ yields the quotient multiset $$\{[M/N_1]\}\cup T=\{[M/N_1],[M/M_1]\}\cup U$$ as well.
we get 
$$
\begin{tikzcd}
&M\ar[dl,dash,"a_1"']\ar[dr,dash,"b_1"]\\ 
