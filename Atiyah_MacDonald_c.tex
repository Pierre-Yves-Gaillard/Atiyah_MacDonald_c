...
(iv)$\implies$(i): For any ideal $\mf a$ of any ring $R$ write $\overline{\mf a}$ for the image of $\mf a$ in $\overline A:=R/\mf N(R)$, define $\overline r\in\overline A$ for $r\in R$ similarly. Let $\mu\in\mf m\subset A$ with $\mf m$ maximal. By Section~\ref{310ii} p.~\pageref{310ii} above, it suffices to show that %there is an $s\in A\setminus\mf m$ such that 
