...
\section{P. 47, Exercise 3.21.iv}\label{321iv}%
...
$$
(A_{\mf p}/\mf p_{\mf p})\otimes_AB\simeq
(A/\mf p)_{\mf p}\otimes_AB\simeq
(A/\mf p)_{\mf p}\otimes_{A/\mf p}A/\mf p\otimes_AB
$$ 
$$
...
\section{Page 53, proof of Proposition 4.8.ii}%
...
Use Exercises 1.18 p.~10 and 1.2ii p.~11.
...
As observed by the Mathematics Stackexchange user Kato here 

\href{https://math.stackexchange.com/q/2808775/660}{https://math.stackexchange.com/q/2808775/660}
...
We denote the Krull dimension of the ring $A$ by $\dim A$.

\subsection{Exercise 11.6}% 

For Exercise 3.21.iv p.~47 of the book see Section~\ref{321iv} p.~\pageref{321iv} above. 

For Exercise 4.7 p.~55 of the book see Section~\ref{5547} p.~\pageref{5547} above.

We'll use the following fact which occurs in the hint to Exercise 11.6 p.~126 of the book:

($\star$) \emph{If $f:A\to A[x]$ is the natural embedding, then the fiber of} 
$$
...
$$
\mf p_0\underset\ne\subset\dots\underset\ne\subset\mf p_n
$$ 
is a chain of prime ideals in $A$, then 
$$
\mf p_0[x]\underset\ne\subset\dots\underset\ne\subset\mf p_n[x]\underset\ne\subset(\mf p,x)
$$ 
is chain of prime ideals in $A[x].\ \square$

\emph{Proof of the inequality} $\dim A[x]\le1+2\dim A$: Statement ($\star$) above implies that, for any chain $\mf p_0\underset\ne\subset\mf p_1\underset\ne\subset\mf p_2$ of prime ideals in $A[x]$, we have:

$\bullet$ if $(\mf p_0)^{\oo c}=(\mf p_1)^{\oo c}$ then $\mf p_0=(\mf p_0)^{\oo c}[x]$ and $(\mf p_1)^{\oo c}\underset\ne\subset(\mf p_2)^{\oo c}$,

$\bullet$ if $(\mf p_1)^{\oo c}=(\mf p_2)^{\oo c}$ then $\mf p_1=(\mf p_1)^{\oo c}[x]$ and $(\mf p_0)^{\oo c}\underset\ne\subset\mf p_1^{\oo c}$.
...
Second sentence of the hint: see Proposition 11.13 p.~121 of the book. 

Inequality $\oo{height }\mf p[x]\le m$: see Corollary 11.16 p.~121 of the book.

Inequality $\dim A[x]\le1+\dim A$ (last sentence of the hint). 

We denote the height of the prime ideal $\mf p$ by $h(\mf p)$.

We give two proofs of the inequality $\dim A[x]\le1+\dim A$.

\emph{Proof 1.} We can assume $\dim A<\infty$. Set $n:=\dim A$ and let 
$$
...
The above chain contracting to a chain of length at most $n$ in $A$, there are indices $i$ such that $(\mf p_i)^{\oo c}=(\mf p_{i+1})^{\oo c}$. Let $i$ be the largest such index. By ($\star$) we have $\mf p_i=(\mf p_i)^{\oo c}[x]$, and thus 
$$
h((\mf p_{i+1})^{\oo c})=h((\mf p_i)^{\oo c})=h(\mf p_i)\ge i.
$$ 
As $(\mf p_{i+2})^{\oo c}$ is strictly larger than $(\mf p_{i+1})^{\oo c}$, we get $h((\mf p_{i+2})^{\oo c})\ge i+1$. Similarly $h((\mf p_{i+3})^{\oo c})\ge i+2$,\dots, and finally $h((\mf p_{n+2})^{\oo c})\ge n+1$, which is impossible. $\square$

\emph{Proof 2.} We prove 
$$
h(\mf p)\le1+h(\mf p^{\oo c})
$$ 
for all prime ideal $\mf p$ of $A[x]$ by induction on $h(\mf p^{\oo c})$.

This will imply $\dim A[x]\le1+\dim A$, as desired.

If $h(\mf p^{\oo c})=0$, the statement follows from ($\star$).

Assume $h(\mf p^{\oo c})>0$. It suffices to show

(a) $h(\mf q)\le h(\mf p^{\oo c})$ for all prime ideal $\mf q$ of $A[x]$ strictly smaller than $\mf p$.

Indeed, there is a chain 
$$
\mf q_0\underset\ne\subset\cdots\underset\ne\subset\mf q_{n-2}\underset\ne\subset\mf q\underset\ne\subset\mf p
$$ 
with $n=h(\mf p)$, and we get $h(\mf q)\ge n-1=h(\mf p)-1$, that is, $h(\mf p)\le1+h(\mf q)$. 
 
Let $\mf q$ be a prime ideal as in (a).

If $\mf q^{\oo c}\underset\ne\subset\mf p^{\oo c}$, we have $h(\mf q^{\oo c})<h(\mf p^{\oo c})$, and the induction hypothesis implies $h(\mf q)\le 1+h(\mf q^{\oo c})\le h(\mf p^{\oo c})$.

If $\mf q^{\oo c}=\mf p^{\oo c}$, we have $\mf q=\mf p^{\oo c}[x]$ by ($\star$). This implies $h(\mf q)=h(\mf p^{\oo c})$. $\square$

%now Exercise 11.6 implies $$\oo{height}(\mf p)\le1+\oo{height}((\mf p)^{\oo c}[x])\le1+\dim A$$ for all prime ideal $\mf p$ of $A[x]$, and thus $\dim A[x]\le1+\dim A$.
