...
Set $k:=i+j$ and define $g:A^k\to A^k$ by $g(x,y):=(f(x,y),0)$ for $x\in A^i$, $y\in A^j$. Note that $g$ is injective. By Proposition~2.4 p.~21 of the book (see Section~\ref{24} p.~\pageref{24} above) there is a monic polynomial $P\in A[x]$ such that there is an $a$ in $A$ with 
...
$$ 
It is clear that $\beta$ is surjective and that $\alpha\circ\beta$ is zero. The flatness of $A/(x)$ implies the injectivity of $\alpha$, and we get $0=(x)\otimes A/(x)\simeq(x)/(x^2)$, hence $(x)^2=(x)$. $\square$
...
If $1\otimes x=0$ in $K\otimes M$, then $1\otimes x=0$ in $A\frac1a\otimes M$. This implies $ax=0$.
...
Claim 1: the natural morphism $A/\ppp^n\to(A/\ppp^n)_\ppp$ is an isomorphism. 

Let $\pi:A\to A/\ppp^n$ be the canonical projection, set $S:=A\setminus\ppp$, and note that $\pi(s)$ is a unit for all $s\in S$ (because $A/\ppp^n$ is a local ring with maximal ideal $\pi(\ppp)$). We leave it to the reader to check that the formula $\phi(a/s)=\pi(s)^{-1}a$ (for $a\in A/\ppp^n$ and $s\in S$) defines a morphism $\phi:(A/\ppp^n)_\ppp\to A/\ppp^n$, and that this morphism is inverse to the natural morphism $A/\ppp^n\to(A/\ppp^n)_\ppp$. This proves Claim~1. 

Claim 2: the natural morphism $M\to M_\ppp$ is an isomorphism. 

We leave it to the reader to check that the formula $\psi(x/s)=\pi(s)^{-1}x$ (for $x\in M$ and $s\in S$) defines a morphism $\psi:M_\ppp\to M$, and that this morphism is inverse to the natural morphism $M\to M_\ppp$. 

In other words the $A$-module $M$ can be viewed as a finitely generated module over $A_\ppp$ annihilated by $\ppp_\ppp^n$. Since $A_\ppp$ is a principal ideal domain and $\ppp_\ppp$ is maximal, there is a unique $k$-tuple $(m_1,\dots,m_k)$ of integers such that 
$$
1\le m_1\le\cdots\le m_k\le n
$$ 
and 
...
$$ 
(isomorphism of $A_\ppp$-modules). This implies $(\star)$. 

%To prove this it suffices to show that the image $\overline s$ in $A/\ppp^n$ of any $s\in A\setminus\ppp$ is a unit. But this is clear because $A/\ppp^n$ is a local ring with maximal ideal $\overline\ppp$ (obvious notation).\\ We claim secondly that the natural morphism $M\to M_\ppp$ is an isomorphism. Indeed we have $$M\simeq A/\ppp^n\otimes_{A/\ppp^n}M\simeq (A/\ppp^n)_\ppp\otimes_{A/\ppp^n}M\simeq M_\ppp. $$ In particular we can regard $M$ is a finitely generated $A_\ppp$-module, and $(\star)$ now follows from the fact that $A_\ppp$ is a principal ideal domain. 
