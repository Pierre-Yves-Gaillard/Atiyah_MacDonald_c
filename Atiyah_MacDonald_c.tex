...
\textbf{Noether Normalization Lemma.} \emph{If $A$ is a domain and $B$ a finitely generated $A$-algebra containing $A$, then there exist a nonzero element $s$ in $A$, a nonnegative integer $n$, and elements $x_1,\dots,x_n$ in $B$ which are algebraically independent over $A$, such that $B_s$ is a finitely generated $A_s[x_1,\dots,x_n]$-module.}

\begin{proof} 
Case 1: $A$ is a field.

Let $y_1,\dots,y_m\in B$. Assume that $B=A[\,y_1,\dots,y_m]$, and that there is a non-constant polynomial $f\in A[Y_1,\dots,Y_m]$, where the $Y_i$ are indeterminates, such that 
$$
f(\,y_1,\dots,y_m)=0.
$$ 
Let $r$ be an integer larger than the degree of $f$. In view of $(\star)$ above, it suffices to show that $B$ is integral over 
$$
C:=A\left[\,y_2-y_1^r,\dots,y_m-y_1^{r^{m-1}}\right].
$$ 
As $B=C[\,y_1]$, it is enough to prove that $y_1$ is integral over $C$. In fact, we only need to check that 

$(\star\star)$ there is an $a\in A^\times$, an integer $d\ge1$, and a 
$$
g\in\bigg(A\left[Y_2-Y_1^r,\dots,Y_m-Y_1^{r^{m-1}}\right]\bigg)[Y_1]
$$ 
of degree less than $d$, such that $f=aY_1^d+g$. 

But this is straightforward. 

More precisely, the key fact used to prove $(\star\star)$ is this: For any monomial $u\in A[Y_1,\dots,Y_m]$ occurring in $f$ (with a nonzero coefficient), let's denote by $u'$ the monomial $u$ \emph{viewed as a polynomial in the ring} 
$$
\bigg(A\left[Y_2-Y_1^r,\dots,Y_m-Y_1^{r^{m-1}}\right]\bigg)[Y_1].
$$ 
Then $u'$ is monic. Moreover, if $u$ and $v$ are two distinct such monomials, then we have $\deg(u')\ne\deg(v')$.

In fact, if $u=Y_1^{\alpha_1}\cdots Y_m^{\alpha_m}$, then $u'$ is monic of degree 
$$
\alpha_1+\alpha_2r+\cdots+\alpha_mr^{m-1}.
$$ 
This completes the proof in the case when $A$ is a field.

Case 2: general case.

Set $S:=A\setminus\{0\}$ and let $K=S^{-1}A$ be the field of fractions of $A$. By the first case, there are elements $x_1,\dots,x_n$ in $S^{-1}B$ which are algebraically independent over $K$, such that $S^{-1}B$ is a finitely generated $K[x_1,\dots,x_n]$-module. We can assume that the $x_i$ are in $B$. Let $y_1,\dots,y_m$ be generators of $B$ as an $A$-algebra, and thus of $S^{-1}B$ as a $K$-algebra. The $y_i$ are integral over $K[x_1,\dots,x_n]$, and it suffices to show that they are integral over $A_s[x_1,\dots,x_n]$ for some $s$ in $S$ (independent of $i$). There exist a positive integer $d$ and elements $u_{ij}\in K[x_1,\dots,x_n]$ such that 
$$
y_i^d+u_{i1}\,y^{i-1}+\cdots+u_{id}=0.
$$ 
Then there is an $s$ in $S$ such that each $u_{ij}$ is of the form $v_{ij}/s$ with $v_{ij}$ in $A[x_1,\dots,x_n]$. This proves that $y_i$ is integral over $A_s[x_1,\dots,x_n]$, as desired. 
\end{proof}
...
(ZL) implies that $I(V)$ is the intersection of all the maximal ideals of $A$ containing $\mf a$. As $r(\mf a)$ is the intersection of all the prime ideals of $A$ containing $\mf a$, we can assume that $\mf a$ is a prime ideal $\mf p$. Let $f$ be in $A\setminus\mf p$ and set $A_f:=A[1/f]$.
