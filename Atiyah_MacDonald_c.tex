...
Let $A$ and $B$ be rings, $B$ integral over $A$; let $\mf q,\mf b$ be ideals of $B$ such that $\mf q$ is prime, $\mf q\subset\mf b$ and $\mf q^{\oo c}=\mf b^{\oo c}=\mf p$ say. Then $\mf q=\mf b$.

Proof. Arguing as in the book, we see that $\mf p_{\mf p}\subset A_{\mf p}$ and $\mf q_{\mf p}\subset B_{\mf p}$ are maximal, and that we have $\mf q_{\mf p}\subset\mf b_{\mf p}\subset B_{\mf p}$. Proposition 3.1ii p.~41 of the book implies $\mf b_{\mf p}\ne B_{\mf p}$, and thus $\mf q_{\mf p}=\mf b_{\mf p}$. If $b$ is in $\mf b$, we get $b/1=q/s$ for some $s$ in $A\setminus\mf p$, then $tb\in\mf q$ for some $t$ in $A\setminus\mf p$, and thus $b\in\mf q$.
