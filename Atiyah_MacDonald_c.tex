...
\section{Page 29, Proposition 2.19}%
...
\section{Page 34, Exercise 2.25, first solution}\label{s225a}%
...
Here is a solution to Exercise 2.25 which does \emph{not} use the Tor functor. Of course, the solution using the Tor functor (and assuming the Tor functor and some of its basic properties are known) is much simpler. In the next section we give a proof which includes a definition of the Tor functor.
...
\section{Page 34, Exercise 2.25, second solution}% doc snake0: https://docs.google.com/document/d/1gTyvvgy4XYRi04V_LI47nxX0mYQvRyn57lMmjo59hlM/edit
...
We return to the natation $A$ for a commutative ring with one. \emph{Module} stands for $A$-\emph{module}. 
...
\section{Page 44, Exercise 3.10i}%

$$a=a^2x\implies\frac{a^2}{s^2}\frac{sx}1=\frac{ax^2s}{s^2}=\frac as\ .$$ 

\section{Page 44, Exercise 3.10ii}\label{310ii}%
...
\section{Page 45, Exercise 3.14}%\label{321iv}
...
\section{Page 45, Exercise 3.15}%\label{321iv}

See Lemma \ref{b254} p. \pageref{b254} above.

\section{Page 46, Exercise 3.20}%\label{321iv}
...
\section{Page 47, Exercise 3.21iv}\label{321iv}%
...
\section{Page 47, Exercise 3.23}%\label{321iv}%
...
\section{Page 53, decomposable ideals}\label{di}%
...
\section{Page 53, proof of Proposition 4.8.ii}\label{prycor}%\label{contrpry}
...
\section{Page 54, Theorem 4.10}%\label{410}%
...
\section{Page 55, Exercise 4.7}\label{5547}%
...
\section{Page 56, Exercise 4.11}\label{411}%
...
\section{Page 56, Exercise 4.16}%

Follows from Proposition 3.11i p.~41 and Proposition 4.9 p.~54.

\section{Page 56, Exercise 4.17}%https://math.stackexchange.com/q/2375591/660 - previous version https://docs.google.com/document/d/1jO2WMu4veaXNHTd_D2QJ89QwvbGW_KaqyJPpb02RU4I/edit

Here is the statement of the exercise:%\centerline{$\star$}£

\emph{Let $A$ be a ring with the following property.}

(L1) \emph{For every ideal $\mathfrak a\ne(1)$ in $A$ and every prime ideal $\mathfrak p$, there exists $x\notin\mathfrak p$ such that $S_{\mathfrak p}(\mathfrak a)=(\mathfrak a:x)$, where} $S_{\mathfrak p}=A\setminus\mathfrak p$.

\emph{Then every ideal in $A$ is an intersection of (possibly infinitely many) primary ideals.}
...
$\bu\ x\in A\setminus\ppp$ satisfies $\mathfrak q=(\bbb:x)$.%(such an $x$ exists by (L1)). 
\end{df}%By Assumption (L1), for all $\bbb\ne(1)$ there is an admissible quadruple $(\bbb,\ppp,\qqq,x)$.
...
$$ 
by Proposition 3.11 p.~41 of the book, and thus $ax\in\bbb$, as desired.%let $q=\beta+ax\in\qqq$ with $\beta\in\bbb,a\in A$. It suffices to prove $q\in\bbb$. Recall that $\qqq=S_{\ppp}(\bbb)=(\bbb:x)$. We get $\beta x+ax^2=qx\in\bbb$, hence $ax^2\in\bbb$, hence $a\in %S_{x}(\bbb)\subset S_{\ppp}(\bbb)=(\bbb:x)$, hence $ax\in\bbb$, and thus $q\in\bbb$.
\end{proof}
...
\end{equation} 
(b) each ordinal $\alpha\in W$ satisfies %\emph{Condition} $\oo C(\alpha)$ if we have 
\begin{equation}\label{e417c}
\aaa_0=\aaa_\alpha\cap\bigcap_{\beta<\alpha}\qqq_\beta.
\end{equation}
We call \eqref{e417c} Condition $\oo C(\alpha)$. 

At each stage Lemma~\ref{l417}b1 will imply that $\qqq_\alpha$ is primary for all $\alpha\in W$. 
...
$\bu$ From $\alpha=$ to $\alpha+1$. Let $\alpha\in W$ be such that $\alpha+1\in W$, and assume that $(\aaa_\beta,\ppp_\beta,\qqq_\beta,x_\beta)$ has already been constructed for $\beta\le\alpha$ and that Condition $\oo C(\alpha)$ holds.%and that $\aaa_\alpha\ne(1)$.

We claim: 
$$
(\aaa_\alpha+(x_\alpha))\cap\bigcap_{\beta\le\alpha}\qqq_\beta=\aaa_0.
$$ 
...
$$ 
by Lemma~\ref{l417}b2. $\square$%Let $q=a_\alpha+bx_\alpha\in\qqq_\alpha$ with $a_\alpha\in\aaa_\alpha,b\in A$. It suffices to prove $q\in\aaa_\alpha$. Recall that $\qqq_\alpha=S_{\ppp_\alpha}(\aaa_\alpha)=(\aaa_\alpha:x_\alpha)$. We get $a_\alpha x_\alpha+bx_\alpha^2=qx_\alpha\in\aaa_\alpha$, hence $bx_\alpha^2\in\aaa_\alpha$, hence $b\in %S_{x_\alpha}(\aaa_\alpha)\subset 
%S_{\ppp_\alpha}(\aaa_\alpha)=(\aaa_\alpha:x_\alpha)$, hence $bx_\alpha\in\aaa_\alpha$, and thus $q\in\aaa_\alpha$. $\square$

By the claim, there is an ideal $\aaa_{\alpha+1}$ containing $\aaa_\alpha+(x_\alpha)$ which is maximal subject to the constraint $\oo C(\alpha+1)$. In particular $\aaa_{\alpha+1}\ne(1)$ by Assumption~\eqref{abs417} and Lemma~\ref{l417}b1. We define $\ppp_{\alpha+1},\qqq_{\alpha+1},x_{\alpha+1}$ in such a way that $(\aaa_{\alpha+1},\ppp_{\alpha+1},\qqq_{\alpha+1},x_{\alpha+1})$ is admissible (Lemma~\ref{l417}a). %If $\aaa_{\alpha+1}=(1)$ we set $\ppp_{\alpha+1}=\ppp_0$, $\qqq_{\alpha+1}=\qqq_0$, $x_{\alpha+1}=x_0$.
...
$$
\left(\bigcup_{\alpha<\gamma}\aaa_\alpha\right)\cap\left(\bigcap_{\beta<\gamma}\qqq_\beta\right)=\aaa_0.
$$ 
Inclusion $\supset$ being clear, it suffices to prove $\subset$. Let $x$ be in the left side. There is an $\alpha<\gamma$ such that %$x$ is in 
$$
x\in\aaa_\alpha\cap\bigcap_{\beta<\gamma}\qqq_\beta\subset\aaa_\alpha\cap\bigcap_{\beta<\alpha}\qqq_\beta=\aaa_0.
$$ 
...
%https://docs.google.com/document/d/1YYUDEGhoImy-b_Lx9foGoetelzzXbN1IiNbpo7pXh98/edit {comment} 180522
%\section{Page 89, Proposition 8.1}%\label{dt}%
...
\section{Page 121, the Dimension Theorem}\label{dt}%
...
\section{Page 126}%% previous version https://docs.google.com/document/d/1pXphjmJVG5y2JSIiE6W0ZBNaBsdVPPTXytsjLOLYc7g/edit
