...
the element $\mu/1$ of $A_{\mf m}$ is nilpotent, which is clear since $\mf m$ is a minimal prime ideal of $A$ (and thus $\mmm_\mmm$ is the nilradical of $A_\mmm$).
...
(b)$\implies$(c): With obvious notation we have $\bar a=\bar a^2\bar b$ for some $b$ in $A$, that is $(a-a^2b)^n=0$ for some $n\ge1$. This is easily seen to imply $a^n\in(a^{n+1})$ and thus $(a^{n+1})=(a^n)$. 
