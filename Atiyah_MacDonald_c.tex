...
%\textbf{Noether Normalization Theorem}\subsubsection{Noether Normalization Theorem}

Before proceeding, recall the following fact (see Remark p.~60 of the book): %it might be useful to spell out a fact which is proved but, it seems to me, not stated explicitly in the book:
...
Recall also the general notation $A_s:=A[s^{-1}]$.%
...
(a) There exist a nonzero $t$ in $A$ and elements $z_1$, \dots, $z_{m-1}$ in $B$ such that $B_t$ is a finitely generated $A_t[z_1,\dots,z_{m-1}]$-module. 

We start the proof of (a). 
...
$$ 
and $B_t$ is a finitely generated $C$-module. This proves (a).

Let us show that (a) implies the Theorem:
...
This shows that, indeed, (a) implies the Theorem.
\end{proof} 
...
Proof of the claim. By contradiction let $0\ne a\in\mf b^{\oo c}$, and let $\mf m$ be a maximal ideal of $A$ avoiding $a$. By Theorem 5.10 p.~62 of the book, there is a prime ideal $\mf n$ of $B$ such lying above $\mf m$. Then $\mf n$ is maximal by Corollary 5.8 p.~61 of the book, and $a\in\mf n\cap A=\mf m$, a contradiction.
