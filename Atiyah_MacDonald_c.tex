...
\documentclass[parskip=half]{scrartcl}%\documentclass[parskip=full]{scrartcl}
...
$$ 
More precisely, let $\pi_i:G\to G/G_i$ be the canonical projection; let $C\subset G^{\mathbb N}$ be the group of Cauchy sequences (this is indeed easily seen to be a subgroup of $G^{\mathbb N}$); note that $x\in G^{\mathbb N}$ is Cauchy if and only if for each $i$ the sequence $j\mapsto\pi_i(x_j)$ is eventually constant, in which case we write $\pi_i(x_\infty)$ for its eventual value; and define 
$$
\varphi:C\to\varprojlim G/G_i
$$ 
by $\varphi(x)_i:=\pi_i(x_\infty)$. Then $\varphi$ is a well defined group epimorphism, and it induces an isomorphism 
$$
